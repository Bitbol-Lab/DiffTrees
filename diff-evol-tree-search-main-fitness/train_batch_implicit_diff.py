...
Field = args['fitness_field'] if args['fitness_field']!=None else False
Coupling = args['fitness_coupling'] if args['fitness_coupling']!=None else False
#CREATING FOLDER WHERE SAVING SIMULATIONS

folder_name="%ileaves_seql%i_%iletters_%imuts_Field_"% (n_leaves, seq_length, n_letters, n_mutations)+str(Field)+"_Coupling_"+str(Coupling)
...
for n_simul in range(1,100):
    ...
    metadata = {
        ...
        'fitness_field' : args['fitness_field'] if args['fitness_field']!=None else False,
        'fitness_coupling' : args['fitness_coupling'] if args['fitness_coupling']!=None else False,
        }
